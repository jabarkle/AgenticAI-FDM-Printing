...
def resume_print(printer_url:str)->str:
    ...
    printer_url = "http://172.26.197.215:81"
...
def query_printer(object_name:str) -> dict:
    ...
    url = f"http://172.26.197.215:81/printer/objects/query?{object_name}"
...
def query_gCode(gcode_command:str) -> dict:
    ...
    Returns:
        dict: The data returned by the Moonraker API.
    """
    endpoint = f"http://172.26.197.215:81/printer/gcode/script"
    ...
    # print(response.json())
    time.sleep(3)
    get_url=f"http://172.26.197.215:81/server/gcode_store?count=1"
